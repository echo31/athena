...
static struct resource asic3_mmc_resources[] = {
	{
		.start = W2284_SD_CTRL_BASE,
		.end   = W2284_SD_CTRL_BASE + 0x3ff,
		.flags = IORESOURCE_MEM,
	},
	{
		.start = 0,
				  ...
				  struct resource *mem)
{
	...
	/* MMC */
	asic->tmio_cnf = ioremap((W2284_SD_CONFIG_BASE >> asic->bus_shift) +
...
static int __init asic3_probe(struct platform_device *pdev)
{
	...
	if (!mem) {
        ...
        printk("W228Xasic: remapping=%x size%x\n",mem->start+0x200000,resource_size(mem));
	asic->mapping = ioremap(mem->start+0x200000, resource_size(mem));
	if (!asic->mapping) {
		ret = -ENOMEM;
