/*
 * linux/drivers/video/w2284fb.c
 *
 * Frame Buffer Device for ATI Imageon w2284 (Wallaby)
 *
 ...
 * Prototypes
 */
static void w2284_suspend(u32 mode);
static void w2284_vsync(void);
static void w2284_hw_init(struct w2284fb_par*);
static void w2284_pwm_setup(struct w2284fb_par*);
static void w2284_init_clocks(struct w2284fb_par*);
static void w2284_setup_memory(struct w2284fb_par*);
static void w2284_init_lcd(struct w2284fb_par*);
static void w2284_set_dispregs(struct w2284fb_par*);
static void w2284_update_enable(void);
static void w2284_update_disable(void);
static void calc_hsync(struct w2284fb_par *par);
static void w2284_init_graphic_engine(struct w2284fb_par *par);
struct w2284_pll_info *w2284_get_xtal_table(unsigned int freq) __devinit;

/* Pseudo palette size */
#define MAX_PALETTES      16

#define W2284_SUSPEND_EXTMEM 0
#define W2284_SUSPEND_ALL    1
...
/* This is the offset in the w2284's address space we map the current
...
#define W2284_FB_BASE MEM_EXT_BASE_VALUE
...
static ssize_t flip_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	struct w2284fb_par *par=info->par;
...
static ssize_t flip_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	...
	w2284_update_disable();
	w2284_set_dispregs(par);
	w2284_update_enable();
...
static ssize_t w2284fb_reg_read(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
...
static DEVICE_ATTR(reg_read, 0200, NULL, w2284fb_reg_read);

static ssize_t w2284fb_reg_write(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	unsigned long regs, param;
...
static DEVICE_ATTR(reg_write, 0200, NULL, w2284fb_reg_write);
...
static ssize_t fastpllclk_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (simple_strtoul(buf, NULL, 10) > 0) {
		par->fastpll_mode=1;
		printk("w2284fb: Using fast system clock (if possible)\n");
	} else {
		par->fastpll_mode=0;
		printk("w2284fb: Using normal system clock\n");
	}

	w2284_init_clocks(par);
	calc_hsync(par);
...
EXPORT_SYMBOL(w2284fb_get_hsynclen);

static void w2284fb_clear_screen(struct w2284fb_par *par)
{
	memset_io(remapped_fbuf + (W2284_FB_BASE-MEM_WINDOW_BASE), 0, (par->xres * par->yres * BITS_PER_PIXEL/8));
}
 ...
 */
static int w2284fb_setcolreg(u_int regno, u_int red, u_int green, u_int blue,
 ...
 */
static int w2284fb_blank(int blank_mode, struct fb_info *info)
{
	struct w2284fb_par *par = info->par;
	struct w2284_tg_info *tg = par->mach->tg;
...
static void w2284_fifo_wait(int entries)
{
	...
	printk(KERN_ERR "w2284fb: FIFO Timeout!\n");
}


static int w2284fb_sync(struct fb_info *info)
{
	union rbbm_status_u status;
	int i;
	...
	printk(KERN_ERR "w2284fb: Graphic engine timeout!\n");
	return -EBUSY;
}
...
static void w2284_init_graphic_engine(struct w2284fb_par *par)
{
	...
	w2284_fifo_wait(4);
	writel(W2284_FB_BASE, remapped_regs + mmDST_OFFSET);
	writel(par->xres, remapped_regs + mmDST_PITCH);
	writel(W2284_FB_BASE, remapped_regs + mmSRC_OFFSET);
...
static void w2284fb_fillrect(struct fb_info *info,
...
static void w2284fb_copyarea(struct fb_info *info,
 ...
 */
static void w2284fb_activate_var(struct w2284fb_par *par)
{
	struct w2284_tg_info *tg = par->mach->tg;

	w2284_pwm_setup(par);
	w2284_setup_memory(par);
	w2284_init_clocks(par);
	w2284fb_clear_screen(par);
	w2284_vsync();
 ...
 */
static struct w2284_mode *w2284fb_get_mode(struct w2284fb_par *par, unsigned int *x, unsigned int *y, int saveval)
{
	struct w2284_mode *mode = NULL;
	struct w2284_mode *modelist = par->mach->modelist;
 ...
 */
static int w2284fb_check_var(struct fb_var_screeninfo *var, struct fb_info *info)
{
	struct w2284fb_par *par=info->par;

	if(!w2284fb_get_mode(par, &var->xres, &var->yres, 1))
		return -EINVAL;
 ...
 */
static int w2284fb_set_par(struct fb_info *info)
{
	struct w2284fb_par *par=info->par;
	...
	if (par->xres != info->var.xres || par->yres != info->var.yres)	{
		...
		par->mode = w2284fb_get_mode(par, &par->xres, &par->yres, 0);
	...
	//	w2284fb_activate_var(par);                  //freeze
	}
	return 0;
}
 ...
 */
static struct fb_ops w2284fb_ops = {
	.owner        = THIS_MODULE,
	.fb_check_var = w2284fb_check_var,
	.fb_set_par   = w2284fb_set_par,
	.fb_setcolreg = w2284fb_setcolreg,
	.fb_blank     = w2284fb_blank,
	.fb_fillrect  = w2284fb_fillrect,
	.fb_copyarea  = w2284fb_copyarea,
	.fb_imageblit = cfb_imageblit,
	.fb_sync      = w2284fb_sync,
};

#ifdef CONFIG_PM
static void w2284fb_save_vidmem(struct w2284fb_par *par)
{
	int memsize;
	...
	if (par->extmem_active) {
		...
		if (par->saved_extmem)
			memcpy_fromio(par->saved_extmem, remapped_fbuf + (W2284_FB_BASE-MEM_WINDOW_BASE), memsize);
	}
	memsize=MEM_INT_SIZE;
	par->saved_intmem = vmalloc(memsize);
	if (par->saved_intmem && par->extmem_active)
		memcpy_fromio(par->saved_intmem, remapped_fbuf + (W2284_FB_BASE-MEM_INT_BASE_VALUE), memsize);
	else if (par->saved_intmem)
		memcpy_fromio(par->saved_intmem, remapped_fbuf + (W2284_FB_BASE-MEM_WINDOW_BASE), memsize);
}

static void w2284fb_restore_vidmem(struct w2284fb_par *par)
{
	int memsize;

	if (par->extmem_active && par->saved_extmem) {
		memsize=par->mach->mem->size;
		memcpy_toio(remapped_fbuf + (W2284_FB_BASE-MEM_WINDOW_BASE), par->saved_extmem, memsize);
		vfree(par->saved_extmem);
	}
	if (par->saved_intmem) {
		memsize=MEM_INT_SIZE;
		if (par->extmem_active)
			memcpy_toio(remapped_fbuf + (W2284_FB_BASE-MEM_INT_BASE_VALUE), par->saved_intmem, memsize);
		else
			memcpy_toio(remapped_fbuf + (W2284_FB_BASE-MEM_WINDOW_BASE), par->saved_intmem, memsize);
		vfree(par->saved_intmem);
	}
}

static int w2284fb_suspend(struct platform_device *dev, pm_message_t state)
{
	struct fb_info *info = platform_get_drvdata(dev);
	struct w2284fb_par *par=info->par;
	struct w2284_tg_info *tg = par->mach->tg;

	w2284fb_save_vidmem(par);
	if(tg && tg->suspend)
		tg->suspend(par);
	w2284_suspend(W2284_SUSPEND_ALL);
	par->blanked = 1;

	return 0;
}

static int w2284fb_resume(struct platform_device *dev)
{
	struct fb_info *info = platform_get_drvdata(dev);
	struct w2284fb_par *par=info->par;
	struct w2284_tg_info *tg = par->mach->tg;
	...
	w2284_hw_init(par);
	w2284fb_activate_var(par);
	w2284fb_restore_vidmem(par);
	if(tg && tg->resume)
		tg->resume(par);
	par->blanked = 0;
...
#else
#define w2284fb_suspend  NULL
#define w2284fb_resume   NULL
#endif


int __devinit w2284fb_probe(struct platform_device *pdev)
{
	int err = -EIO;
	struct w2284fb_mach_info *inf;
	struct fb_info *info = NULL;
	struct w2284fb_par *par;
	struct resource *mem = platform_get_resource(pdev, IORESOURCE_MEM, 0);
	unsigned int chip_id;
        unsigned int intmem_size;
	...
	remapped_base = ioremap_nocache(mem->start+W2284_CFG_BASE, W2284_CFG_LEN);
	if (remapped_base == NULL)
		goto out;

	/* Map the register space */
	remapped_regs = ioremap_nocache(mem->start+W2284_REG_BASE, W2284_REG_LEN);
	...
	chip_id = readl(remapped_regs + mmCHIP_ID);
        intmem_size = (((readl(remapped_regs + mmMC_FB_LOCATION) & 0xFFFF0000) + 0x00010000 ) >> 8) - ((readl(remapped_regs +       mmMC_FB_LOCATION) & 0x0000FFFF) << 8);
        
        extmem_size = (((readl(remapped_regs + mmMC_EXT_MEM_LOCATION) & 0xFFFF0000) + 0x00010000 ) >> 8) - ((readl(remapped_regs + mmMC_EXT_MEM_LOCATION) & 0x0000FFFF) << 8);
	switch(chip_id) {
		case CHIP_ID_W2282: printk("ATI IMAGEON W2282"); break;
		case CHIP_ID_W2284: printk("ATI IMAGEON W2284"); break;
		default:
			printk("Unknown imageon chip ID\n");
			err = -ENODEV;
			goto out;
	}
	printk(" at 0x%08x.\n", mem->start + W2284_CFG_BASE);
        printk(KERN_INFO "Chip ID:%x", chip_id);
        printk(KERN_INFO "Chip REVISION:%x", readl(remapped_regs + mmREVISION_ID));
        printk(KERN_INFO "Chip INT MEMORY:%dkB\n", intmem_size/1024);
        printk(KERN_INFO "Chip EXT MEMORY:%dkB\n", extmem_size/1024);
        printk(KERN_INFO "Remaped base adress:0x%p", remapped_base);
        printk(KERN_INFO "Remaped regs adress:0x%p", remapped_regs);

	/* Remap the framebuffer */
	remapped_fbuf = ioremap_nocache(mem->start+W2284_FB_BASE, MEM_WINDOW_SIZE);
	if (remapped_fbuf == NULL)
		goto out;

	info=framebuffer_alloc(sizeof(struct w2284fb_par), &pdev->dev);
	if (!info) {
		err = -ENOMEM;
		goto out;
	}
	...
	par->pll_table=w2284_get_xtal_table(inf->xtal_freq);
	if (!par->pll_table) {
	...
	strcpy(info->fix.id, "w2284fb");
	info->fix.type = FB_TYPE_PACKED_PIXELS;
	info->fix.type_aux = 0;
	info->fix.accel = FB_ACCEL_NONE;
	info->fix.smem_start = mem->start + W2284_FB_BASE;
	info->fix.mmio_start = mem->start + W2284_REG_BASE;
	info->fix.mmio_len = W2284_REG_LEN;
...
//	info->fix.ypanstep	= 1;
 	...
 	w2284_hw_init(par);
         printk(KERN_INFO "Regs:%x", readl(remapped_regs + mmCIF_CNTL));
         printk(KERN_INFO "Regs:%x", readl(remapped_regs + mmCIF_WRITE_DBG));
         printk(KERN_INFO "Regs:%x", readl(remapped_regs + mmCIF_READ_DBG));
          printk(KERN_INFO "Regs:%x", readl(remapped_regs + mmCLK_PIN_CNTL));
      
printk(KERN_INFO "w2284fb: framebuffer at 0x%lx, mapped to 0x%p, size %dk\n", info->fix.smem_start, info->screen_base, info->fix.smem_len/1024);
	

	if (w2284fb_check_var(&info->var, info) < 0) {
		err = -EINVAL;
		goto out;
	}
...
static int __devexit w2284fb_remove(struct platform_device *pdev)
{
...
static void w2284_soft_reset(void)
{
...
EXPORT_SYMBOL(w2284fb_gpio_read);
EXPORT_SYMBOL(w2284fb_gpio_write);

/*
 * Initialization of critical w2284 hardware
 */
static void w2284_hw_init(struct w2284fb_par *par)
{
	u32 temp32;
	...
	struct w2284_gpio_regs *gpio = par->mach->gpio;
	...
	writeb((u8) (w2284_REG_BASE >> 16), remapped_base + cfgREG_BASE);
#endif
	cfgreg_base.val = defCFGREG_BASE;
	cfgreg_base.f.cfgreg_base = W2284_CFG_BASE;
	...
	writel((u32) (wrap_top_dir.val), remapped_regs + mmWRAP_TOP_DIR);
        ...
        writel((u32) 0x244, remapped_regs + mmRBBM_1);
	//writel((u32) 0x2440, remapped_regs + mmRBBM_CNTL);
 
         writel(0x00110001,remapped_base + 0x00000008);
        writel(0x000e0017,remapped_base + 0x0000000c);
        writel(0x00000677, remapped_base + 0x00000018);
        writel(0x0000ce7d,remapped_base + 0x0000001c);
        writel(0x00000010,remapped_base + 0x00000028);
        writel(0x00000001,remapped_base + 0x00000034);
...
static struct power_state w2284_pwr_state;

/* The PLL Fout is determined by (XtalFreq/(M+1)) * ((N_int+1) + (N_fac/8)) */

static struct w2284_pll_info xtal_7045120[] = {
	/*freq     M   N_int    N_fac  tfgoal  lock_time */
	{ 60,      0,   27,      0,     0xe0,         5},  /*  50.00 MHz */
        {  0,      0,   0,       0,        0,         0},  /* Terminator */
};

/* 12.5MHz Crystal PLL Table */
static struct w2284_pll_info xtal_12500000[] = {
	/*freq     M   N_int    N_fac  tfgoal  lock_time */
...
static struct w2284_pll_info xtal_14318000[] = {
	/*freq     M   N_int    N_fac  tfgoal  lock_time */
...
static struct w2284_pll_info xtal_16000000[] = {
	/*freq     M   N_int    N_fac  tfgoal  lock_time */
...
struct w2284_pll_info __devinit *w2284_get_xtal_table(unsigned int freq)
{
	struct pll_entries *pll_entry = w2284_pll_tables;
...
static unsigned int w2284_get_testcount(unsigned int testclk_sel)
{
...
static int w2284_pll_adjust(struct w2284_pll_info *pll)
{
	unsigned int tf80;
	unsigned int tf20;

	/* Initial Settings */
	w2284_pwr_state.pll_cntl.f.pll_pwdn = 0x0;     /* power down */
	w2284_pwr_state.pll_cntl.f.pll_reset = 0x0;    /* not reset */
	w2284_pwr_state.pll_cntl.f.pll_tcpoff = 0x1;   /* Hi-Z */
	w2284_pwr_state.pll_cntl.f.pll_pvg = 0x0;      /* VCO gain = 0 */
	w2284_pwr_state.pll_cntl.f.pll_vcofr = 0x0;    /* VCO frequency range control = off */
	w2284_pwr_state.pll_cntl.f.pll_ioffset = 0x0;  /* current offset inside VCO = 0 */
	w2284_pwr_state.pll_cntl.f.pll_ring_off = 0x0;
	...
	do {
		...
		w2284_pwr_state.pll_cntl.f.pll_dactal = 0xd;
		writel((u32) (w2284_pwr_state.pll_cntl.val), remapped_regs + mmPLL_CNTL);

		tf80 = w2284_get_testcount(TESTCLK_SRC_PLL);
		if (tf80 >= (pll->tfgoal)) {
			/* set VCO input = 0.2 * VDD */
			w2284_pwr_state.pll_cntl.f.pll_dactal = 0x7;
			writel((u32) (w2284_pwr_state.pll_cntl.val), remapped_regs + mmPLL_CNTL);

			tf20 = w2284_get_testcount(TESTCLK_SRC_PLL);
			if (tf20 <= (pll->tfgoal))
				return 1;  /* Success */

			if ((w2284_pwr_state.pll_cntl.f.pll_vcofr == 0x0) &&
				((w2284_pwr_state.pll_cntl.f.pll_pvg == 0x7) ||
				(w2284_pwr_state.pll_cntl.f.pll_ioffset == 0x0))) {
				/* slow VCO config */
				w2284_pwr_state.pll_cntl.f.pll_vcofr = 0x1;
				w2284_pwr_state.pll_cntl.f.pll_pvg = 0x0;
				w2284_pwr_state.pll_cntl.f.pll_ioffset = 0x0;
				continue;
			}
		}
		if ((w2284_pwr_state.pll_cntl.f.pll_ioffset) < 0x3) {
			w2284_pwr_state.pll_cntl.f.pll_ioffset += 0x1;
		} else if ((w2284_pwr_state.pll_cntl.f.pll_pvg) < 0x7) {
			w2284_pwr_state.pll_cntl.f.pll_ioffset = 0x0;
			w2284_pwr_state.pll_cntl.f.pll_pvg += 0x1;
		} else {
			return 0;  /* Error */
		}
	} while(1);
...
/*
 * w2284_pll_calibration
 */
static int w2284_pll_calibration(struct w2284_pll_info *pll)
{
	int status;

	status = w2284_pll_adjust(pll);

	/* PLL Reset And Lock */
	/* set VCO input = 0.5 * VDD */
	w2284_pwr_state.pll_cntl.f.pll_dactal = 0xa;
	writel((u32) (w2284_pwr_state.pll_cntl.val), remapped_regs + mmPLL_CNTL);

	udelay(1);  /* reset time */

	/* enable charge pump */
	w2284_pwr_state.pll_cntl.f.pll_tcpoff = 0x0;  /* normal */
	writel((u32) (w2284_pwr_state.pll_cntl.val), remapped_regs + mmPLL_CNTL);

	/* set VCO input = Hi-Z, disable DAC */
	w2284_pwr_state.pll_cntl.f.pll_dactal = 0x0;
	writel((u32) (w2284_pwr_state.pll_cntl.val), remapped_regs + mmPLL_CNTL);
...
static int w2284_pll_set_clk(struct w2284_pll_info *pll)
{
	int status;

	if (w2284_pwr_state.auto_mode == 1)  /* auto mode */
	{
		w2284_pwr_state.pwrmgt_cntl.f.pwm_fast_noml_hw_en = 0x0;  /* disable fast to normal */
		w2284_pwr_state.pwrmgt_cntl.f.pwm_noml_fast_hw_en = 0x0;  /* disable normal to fast */
		writel((u32) (w2284_pwr_state.pwrmgt_cntl.val), remapped_regs + mmPWRMGT_CNTL);
	}

	/* Set system clock source to XTAL whilst adjusting the PLL! */
	w2284_pwr_state.sclk_cntl.f.sclk_src_sel = CLK_SRC_XTAL;
	writel((u32) (w2284_pwr_state.sclk_cntl.val), remapped_regs + mmSCLK_CNTL);

	w2284_pwr_state.pll_ref_fb_div.f.pll_ref_div = pll->M;
	w2284_pwr_state.pll_ref_fb_div.f.pll_fb_div_int = pll->N_int;
	w2284_pwr_state.pll_ref_fb_div.f.pll_fb_div_frac = pll->N_fac;
	w2284_pwr_state.pll_ref_fb_div.f.pll_lock_time = pll->lock_time;
	writel((u32) (w2284_pwr_state.pll_ref_fb_div.val), remapped_regs + mmPLL_REF_FB_DIV);

	w2284_pwr_state.pwrmgt_cntl.f.pwm_mode_req = 0;
	writel((u32) (w2284_pwr_state.pwrmgt_cntl.val), remapped_regs + mmPWRMGT_CNTL);

	status = w2284_pll_calibration(pll);

	if (w2284_pwr_state.auto_mode == 1)  /* auto mode */
	{
		w2284_pwr_state.pwrmgt_cntl.f.pwm_fast_noml_hw_en = 0x1;  /* reenable fast to normal */
		w2284_pwr_state.pwrmgt_cntl.f.pwm_noml_fast_hw_en = 0x1;  /* reenable normal to fast  */
		writel((u32) (w2284_pwr_state.pwrmgt_cntl.val), remapped_regs + mmPWRMGT_CNTL);
	}
	return status;
}
...
static int w2284_set_pll_freq(struct w2284fb_par *par, unsigned int freq)
{
	struct w2284_pll_info *pll = par->pll_table;

	do {
		if (freq == pll->freq) {
			return w2284_pll_set_clk(pll);
		}
		pll++;
	} while(pll->freq);
	return 0;
...
static void w2284_pwm_setup(struct w2284fb_par *par)
{
	w2284_pwr_state.clk_pin_cntl.f.osc_en = 0x1;
	w2284_pwr_state.clk_pin_cntl.f.osc_gain = 0x1f;
	w2284_pwr_state.clk_pin_cntl.f.dont_use_xtalin = 0x0;
	w2284_pwr_state.clk_pin_cntl.f.xtalin_pm_en = 0x0;
	w2284_pwr_state.clk_pin_cntl.f.xtalin_dbl_en = par->mach->xtal_dbl ? 1 : 0;
	w2284_pwr_state.clk_pin_cntl.f.cg_debug = 0x0;
	writel((u32) (w2284_pwr_state.clk_pin_cntl.val), remapped_regs + mmCLK_PIN_CNTL);

	w2284_pwr_state.sclk_cntl.f.sclk_src_sel = 0x1; //CLK_SRC_XTAL;
	w2284_pwr_state.sclk_cntl.f.sclk_turbo_ppl_src_divider = 0x5;  /* turbo divider = 6 */
	w2284_pwr_state.sclk_cntl.f.sclk_fast_ppl_src_divider = 0x3;    /* fast divider = 4 */
	w2284_pwr_state.sclk_cntl.f.sclk_normal_ppl_src_divider = 0x8;  /* normal divider = 9 */
	writel((u32) (w2284_pwr_state.sclk_cntl.val), remapped_regs + mmSCLK_CNTL);

	w2284_pwr_state.pclk_cntl.f.pclk_pix_src = CLK_SRC_PLL; //CLK_SRC_XTAL;
	w2284_pwr_state.pclk_cntl.f.pclk_pix_div = 0x8;    /* P = 9 */
	w2284_pwr_state.pclk_cntl.f.pclk_force_disp = 0x0;  /* Dynamic */
	writel((u32) (w2284_pwr_state.pclk_cntl.val), remapped_regs + mmPCLK_CNTL);

	w2284_pwr_state.pll_ref_fb_div.f.pll_ref_div = 0x0;     /* M = 1 */
	w2284_pwr_state.pll_ref_fb_div.f.pll_fb_div_int = 0x0;  /* N = 1.0 */
	w2284_pwr_state.pll_ref_fb_div.f.pll_fb_div_frac = 0x0;
	w2284_pwr_state.pll_ref_fb_div.f.pll_reset_time = 0x5;
	w2284_pwr_state.pll_ref_fb_div.f.pll_lock_time = 0xff;
	writel((u32) (w2284_pwr_state.pll_ref_fb_div.val), remapped_regs + mmPLL_REF_FB_DIV);

	w2284_pwr_state.pll_cntl.f.pll_pwdn = 0x1;
	w2284_pwr_state.pll_cntl.f.pll_reset = 0x1;
	w2284_pwr_state.pll_cntl.f.pll_pm_en = 0x0;
	w2284_pwr_state.pll_cntl.f.pll_mode = 0x0;  /* uses VCO clock */
	w2284_pwr_state.pll_cntl.f.pll_refclk_sel = 0x0;
	w2284_pwr_state.pll_cntl.f.pll_fbclk_sel = 0x0;
	w2284_pwr_state.pll_cntl.f.pll_tcpoff = 0x0;
	w2284_pwr_state.pll_cntl.f.pll_pcp = 0x4;
	w2284_pwr_state.pll_cntl.f.pll_pvg = 0x0;
	w2284_pwr_state.pll_cntl.f.pll_vcofr = 0x0;
	w2284_pwr_state.pll_cntl.f.pll_ioffset = 0x0;
	w2284_pwr_state.pll_cntl.f.pll_pecc_mode = 0x0;
	w2284_pwr_state.pll_cntl.f.pll_pecc_scon = 0x0;
	w2284_pwr_state.pll_cntl.f.pll_dactal = 0x0;  /* Hi-Z */
	w2284_pwr_state.pll_cntl.f.pll_cp_clip = 0x3;
	w2284_pwr_state.pll_cntl.f.pll_conf = 0x2;
	w2284_pwr_state.pll_cntl.f.pll_mbctrl = 0x2;
	w2284_pwr_state.pll_cntl.f.pll_ring_off = 0x0;
	writel((u32) (w2284_pwr_state.pll_cntl.val), remapped_regs + mmPLL_CNTL);

	w2284_pwr_state.pwrmgt_cntl.f.pwm_enable = 0x0;
	w2284_pwr_state.pwrmgt_cntl.f.pwm_mode_req = 0x1;  /* normal mode (0, 1, 3) */
	w2284_pwr_state.pwrmgt_cntl.f.pwm_wakeup_cond = 0x0;
	w2284_pwr_state.pwrmgt_cntl.f.pwm_fast_noml_hw_en = 0x0;
	w2284_pwr_state.pwrmgt_cntl.f.pwm_noml_fast_hw_en = 0x0;
	w2284_pwr_state.pwrmgt_cntl.f.pwm_fast_noml_cond = 0x1;  /* PM4,ENG */
	w2284_pwr_state.pwrmgt_cntl.f.pwm_noml_fast_cond = 0x1;  /* PM4,ENG */
	w2284_pwr_state.pwrmgt_cntl.f.pwm_idle_timer = 0xFF;
	w2284_pwr_state.pwrmgt_cntl.f.pwm_busy_timer = 0xFF;
	writel((u32) (w2284_pwr_state.pwrmgt_cntl.val), remapped_regs + mmPWRMGT_CNTL);

	w2284_pwr_state.auto_mode = 0;  /* manual mode */
}


/*
 * Setup the w2284 clocks for the specified mode
 */
static void w2284_init_clocks(struct w2284fb_par *par)
{
	struct w2284_mode *mode = par->mode;

	if (mode->pixclk_src == CLK_SRC_PLL || mode->sysclk_src == CLK_SRC_PLL)
		w2284_set_pll_freq(par, (par->fastpll_mode && mode->fast_pll_freq) ? mode->fast_pll_freq : mode->pll_freq);

	w2284_pwr_state.sclk_cntl.f.sclk_src_sel = mode->sysclk_src;
        w2284_pwr_state.sclk_cntl.f.sclk_turbo_ppl_src_divider = mode->sysclk_divider;  /* turbo divider = 6 */
	w2284_pwr_state.sclk_cntl.f.sclk_fast_ppl_src_divider =  mode->sysclk_divider;    /* fast divider = 4 */
	w2284_pwr_state.sclk_cntl.f.sclk_normal_ppl_src_divider = mode->sysclk_divider;  /* normal divider = 9 */	
	writel((u32) (w2284_pwr_state.sclk_cntl.val), remapped_regs + mmSCLK_CNTL);
}

static void w2284_init_lcd(struct w2284fb_par *par)
{
	u32 temp32;
	struct w2284_mode *mode = par->mode;
	struct w2284_gen_regs *regs = par->mach->regs;
...
static void w2284_setup_memory(struct w2284fb_par *par)
{
	union mc_ext_mem_location_u extmem_location;
	union mc_fb_location_u intmem_location;
	struct w2284_mem_info *mem = par->mach->mem;
	struct w2284_bm_mem_info *bm_mem = par->mach->bm_mem;

	if (!par->extmem_active) {
		w2284_suspend(W2284_SUSPEND_EXTMEM);

		/* Map Internal Memory at FB Base */
		intmem_location.f.mc_fb_start = W2284_FB_BASE >> 8;
		intmem_location.f.mc_fb_top = (W2284_FB_BASE+MEM_INT_SIZE) >> 8;
		...
		extmem_location.f.mc_ext_mem_start = W2284_FB_BASE >> 8;
		extmem_location.f.mc_ext_mem_top = (W2284_FB_BASE+par->mach->mem->size) >> 8;
		writel((u32) (extmem_location.val), remapped_regs + mmMC_EXT_MEM_LOCATION);
...
static void w2284_set_dispregs(struct w2284fb_par *par)
{
	...
	switch (par->chip_id) {
		case CHIP_ID_W2282:
			graphic_ctrl.f_w2284.color_depth=6;
			graphic_ctrl.f_w2284.en_crtc=1;
			graphic_ctrl.f_w2284.en_graphic_req=1;
			graphic_ctrl.f_w2284.en_graphic_crtc=1;
			graphic_ctrl.f_w2284.lcd_pclk_on=1;
			graphic_ctrl.f_w2284.lcd_sclk_on=1;
			graphic_ctrl.f_w2284.low_power_on=0;
			graphic_ctrl.f_w2284.req_freq=0;
			graphic_ctrl.f_w2284.portrait_mode=rot;
			...
			switch(par->xres) {
				...
				case 320:
				default:
					graphic_ctrl.f_w2284.total_req_graphic=0xa0;
					break;
				case 480:
				case 640:
					switch(rot) {
						case 0:  /* 0 */
						case 3:  /* 180 */
							graphic_ctrl.f_w2284.low_power_on=1;
							graphic_ctrl.f_w2284.req_freq=5;
						break;
						case 1:  /* 90 */
						case 2:  /* 270 */
							graphic_ctrl.f_w2284.req_freq=4;
							break;
						default:
							break;
					}
					graphic_ctrl.f_w2284.total_req_graphic=0xf0;
					break;
			}
			break;
		case CHIP_ID_W2284:
			...
			graphic_ctrl.f_w2284.low_power_on=0;
			graphic_ctrl.f_w2284.req_freq=0;
			graphic_ctrl.f_w2284.total_req_graphic=par->mode->xres >> 1; /* panel xres, not mode */
			graphic_ctrl.f_w2284.portrait_mode=rot;
			break;
	}

	/* Set the pixel clock source and divider */
	w2284_pwr_state.pclk_cntl.f.pclk_pix_src = par->mode->pixclk_src;
	w2284_pwr_state.pclk_cntl.f.pclk_pix_div = divider;
	writel((u32) (w2284_pwr_state.pclk_cntl.val), remapped_regs + mmPCLK_CNTL);

	writel(graphic_ctrl.val, remapped_regs + mmGRAPHIC_CTRL);
	writel(W2284_FB_BASE + ((offset * BITS_PER_PIXEL/8)&~0x03UL), remapped_regs + mmGRAPHIC_OFFSET);
...
static void calc_hsync(struct w2284fb_par *par)
{
	...
	hsync /= (w2284_pwr_state.pclk_cntl.f.pclk_pix_div + 1);
...
//	writel(0xBFFF8000, remapped_regs + mmMC_EXT_MEM_LOCATION);
//	writel(0x00FF0000, remapped_regs + mmMC_PERF_MON_CNTL);
        writel(0x00000505, remapped_regs + mmRBBM_CMDFIFO_STAT);
        writel(0x401f0019, remapped_regs + 0x0240);
        writel(0x00000001, remapped_regs + 0x0244);
        writel(0x0,        remapped_regs + 0x0444);
        writel(0x003fffff, remapped_regs + 0x0494);
        writel(0x0000203b, remapped_regs + 0x04b0);
        writel(0x00000060, remapped_regs + 0x04b4);
        writel(0x000f000f, remapped_regs + 0x04d8);
        writel(0x00030003, remapped_regs + 0x04e4);
        writel(0x001700c0, remapped_regs + 0x05c0);
        writel(0x07ccb2cb, remapped_regs + 0x05c4);
# if 0
...
static struct platform_driver w2284fb_driver = {
	.probe		= w2284fb_probe,
	.remove		= __devexit_p(w2284fb_remove),
	.suspend	= w2284fb_suspend,
	.resume		= w2284fb_resume,
	.driver		= {
		.name	= "w2284fb",
	},
};

int __init w2284fb_init(void)
{
	return platform_driver_register(&w2284fb_driver);
}

void __exit w2284fb_cleanup(void)
{
	platform_driver_unregister(&w2284fb_driver);
}

module_init(w2284fb_init);
module_exit(w2284fb_cleanup);

//MODULE_DESCRIPTION("ATI Imageon w2284 framebuffer driver");
//MODULE_LICENSE("GPL");
